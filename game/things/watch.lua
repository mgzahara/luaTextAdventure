...
function watchinit()
	...
	mapping["watch"] = {[0] = myNumber, [1] = 0};
	mapping["rolex"] = {[0] = myNumber, [1] = 0};
	mapping["Rolex"] = {[0] = myNumber, [1] = 0};
	
	items[myNumber] = {[0] = "stylish watch", [1] = 13, [2] = 0, [3] = "a", [4] = useWatch};
...
function useWatch(arg, target)
	local mynum = mapping["watch"][0];
	...
	elseif (arg == 20) then -- use
		if(myStuff[1] == 0) then -- in my inverntory
			handlers[24]() -- call time function
			return 0
		elseif(myStuff[1] ==  location) then -- in this room
			print("you are not holding "..myStuff[3].." "..myStuff[0])
			return 0
