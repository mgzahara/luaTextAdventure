...
function belgradeTicketinit()
	local myNumber = nextNumber;
	nextNumber = nextNumber + 1;
	
	mapping["Belgrade ticket"] = {[0] = myNumber, [1] = 0};
	
	items[myNumber] = {[0] = "ticket to Belgrade", [1] = 53, [2] = 0, [3] = "a", [4] = useBelgradeTicket};
...
function useBelgradeTicket(arg, target)
-- arg == verb used on this item
--target == a person
	local mynum = mapping["Belgrade ticket"][0];
	...
	elseif (arg == 20) then -- use
		if(myStuff[1] == 0) then -- in my inverntory
			handlers[23]("Belgrade Airport") -- call fly function
			return 0
		elseif(myStuff[1] ==  location) then -- in this room
			print("you are not holding "..myStuff[3].." "..myStuff[0])
			return 0
