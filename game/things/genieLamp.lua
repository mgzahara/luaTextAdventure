...
function genieLampinit()
	local myNumber = nextNumber;
	nextNumber = nextNumber + 1;
	
	mapping["magic lamp"] = {[0] = myNumber, [1] = 0};
	mapping["mystic lamp"] = {[0] = myNumber, [1] = 0};
	mapping["lamp"] = {[0] = myNumber, [1] = 0};
	
	items[myNumber] = {[0] = "strange lamp", [1] = 60, [2] = 0, [3] = "a", [4] = useMagicLamp};
...
function useMagicLamp(arg, target)
-- arg == verb used on this item
--target == ???
	local mynum = mapping["magic lamp"][0];
	...
	elseif (arg == 21) then -- rub
		if(myStuff[1] == 0) then -- in my inverntory
			if(location == 3 and (days % 2) == 1) then -- in smiths office at night
				print("a genie pops out of the lamp... he looks annoyed")
				print("he asks you what you want to wish for")
				local str = io.read()
				while(str == "tBell everywhere!") do
					print("the genie refuses to grant your wish... maybe try a different one")
					local str = io.read()
				end
				print("\nthanks to that weird genie, eveyone in the world has access to a Taco Bell")
				print("you have beaten this game... good job")
				location = 100
			else
