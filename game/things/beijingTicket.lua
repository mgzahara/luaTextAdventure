...
function beijingTicketinit()
	local myNumber = nextNumber;
	nextNumber = nextNumber + 1;
	
	mapping["Beijing ticket"] = {[0] = myNumber, [1] = 0};
	
	items[myNumber] = {[0] = "ticket to Beijing", [1] = 55, [2] = 0, [3] = "a", [4] = useBeijingTicket};
...
function useBeijingTicket(arg, target)
-- arg == verb used on this item
--target == ???
	local mynum = mapping["Beijing ticket"][0];
	...
	elseif (arg == 20) then -- use
		if(myStuff[1] == 0) then -- in my inverntory
			handlers[23]("Beijing Airport") -- call fly function
			return 0
		elseif(myStuff[1] ==  location) then -- in this room
			print("you are not holding "..myStuff[3].." "..myStuff[0])
			return 0
