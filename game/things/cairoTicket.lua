...
function cairoTicketinit()
	local myNumber = nextNumber;
	nextNumber = nextNumber + 1;
	
	mapping["Cairo ticket"] = {[0] = myNumber, [1] = 0};
	
	items[myNumber] = {[0] = "ticket to Cairo", [1] = 52, [2] = 0, [3] = "a", [4] = useCairoTicket};
...
function useCairoTicket(arg, target)
-- arg == verb used on this item
--target == ???
	local mynum = mapping["Cairo ticket"][0];
	...
	elseif (arg == 20) then -- use
		if(myStuff[1] == 0) then -- in my inverntory
			handlers[23]("Cairo Airport") -- call fly function
			return 0
		elseif(myStuff[1] ==  location) then -- in this room
			print("you are not holding "..myStuff[3].." "..myStuff[0])
			return 0
