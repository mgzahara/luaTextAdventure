...
function tomislavinit() -- serbia president
	local myNumber = nextNumber;
	nextNumber = nextNumber + 1;
	
	mapping["tomislav"] = {[0] = myNumber, [1] = 1};
	mapping["tomislav nikolic"] = {[0] = myNumber, [1] = 1};
	mapping["Tomislav"] = {[0] = myNumber, [1] = 1};
	mapping["Tomislav Nikolic"] = {[0] = myNumber, [1] = 1};
	
	people[myNumber] = {[0] = "Tomislav Nikolic", [1] = 26, [2] = 0, [3] = talkTomislav, [4] = tickTomislav};
...
function talkTomislav(arg, target)

	local mynum = mapping["tomislav"][0];
	...
	if(target)then -- target was passed
		...
		elseif(arg == 33)then -- give
			if(target[0] == "tasty taco")then -- give taco
				if(target[1] == 0)then
					if(events["serbiaMandate"])then -- shown mandate to Xi
						events["serbiaTaco"] = true
